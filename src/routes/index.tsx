...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Цех пересборки Елены Кремневой — пересборка бизнес-модели" },
      { name: "description", content: "Практический цех пересборки в регионе. Найдите скрытые источники дохода и вырастите без круглосуточной пахоты." },
      { property: "og:title", content: "Цех пересборки Елены Кремневой" },
...
const comparison = [
  {
    ...
    mba: "Вам дают масштабные кейсы международных гигантов вроде IKEA или Леруа Мерлен. Но бизнес-практики мировых корпораций абсолютно не применимы для розницы или производства в регионе.",
...
const fitPoints = [
  ...
  { title: "Ищете решения для своего города", desc: "Понимаете, что вам нужны инструменты, адаптированные строго под конъюнктуру региона, а не абстрактные примеры из жизни московских корпораций." },
  ...
  { title: "Не готовы переплачивать за бренд", desc: "Понимаете, что дорогие программы федеральных бизнес-школ с цензом входа от 100 млн рублей просто не соответствуют масштабу вашего текущего бизнеса." },
...
function InvitePopupInner() {
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem("invitePopupShown") === "1") return;
    const t = setTimeout(() => {
      setOpen(true);
      ...
      <Container className="py-12 md:py-28">
        <div className="max-w-3xl">
          ...
          <p className="mt-6 text-lg leading-relaxed text-muted-foreground md:text-xl">
            Путь собственника в цехе — пять последовательных шагов, которые превращают
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-primary p-8 text-primary-foreground shadow-2xl md:p-12">
              ...
              <div className="relative z-10">
                ...
                <p className="max-w-3xl text-lg font-light leading-relaxed text-primary-foreground/85 md:text-xl">
                  Пошаговая бизнес-стратегия от преподавателей, предпринимателей и практиков нашего цеха пересборки бизнеса,
...
function Founder() {
  const quotes = [
    "21 год рядом с региональными предпринимателями (с 2005 года) — юридические консультации, налоговые риски, реальные провалы и реальные взлёты бизнеса.",
    "15 лет академического преподавания. Умею объяснять сложное простым языком — так, чтобы у собственника в голове щёлкнуло.",
    "Опыт MBA показал: дают набор предметов, но не дают сборку именно твоей бизнес-модели под реально малый бизнес в регионе. Мы адаптировали мировые бизнес-практики под региональный бизнес.",
        ...
        <div className="grid gap-10 lg:grid-cols-[5fr_7fr] lg:gap-16 lg:items-center">
          ...
          <div>
            ...
            <p className="mt-4 text-lg text-neutral-600">
              Основатель цеха пересборки. Юрист, методолог, преподаватель. Собирает программу из того, что реально работает у региональных собственников.
            </p>
            <ul className="mt-8 space-y-5">
              {quotes.map((q, i) => (
                <li key={i} className="relative rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
                  <span className="absolute -left-2 top-5 h-8 w-1 rounded-full bg-gradient-to-b from-[#059669] to-[#0f766e]" />
                  <p className="text-base leading-relaxed text-neutral-800">«{q}»</p>
                </li>
              ))}
            </ul>
            <a
...
function Teachers() {
  const items = [
    { name: "Елена Кремнева", role: "Основатель", note: "Кандидат юридических наук. Выпускница Executive MBA Московской школы управления «Сколково». Бизнес-практик, архитектор бизнес-моделей.", img: elenaAsset.url },
    ...
    { name: "Александра Гречушенко", role: "Ведущий тренинга", note: "Соучредитель ООО «Ментор Центр». Бизнес-психолог, ментор, коуч, бизнес-тренер. Работает с предпринимателями на стыке психологии управления и бизнес-результатов.", img: aleksandraAsset.url },
          ...
          <div className="relative">
            ...
            <ul className="mt-8 grid gap-3 md:grid-cols-2">
              {[
                ...
                {
                  title: "Диплом о ДПО и сертификат о ДО",
                  sub: "Документы установленного образца по итогам программы",
                },
              ].map((item) => (
                <li
                  key={item.title}
      ...
      <Container className="grid gap-12 md:grid-cols-2 md:items-center">
        ...
        <div>
          ...
          <p className="mt-6 rounded-2xl border-l-4 border-primary bg-secondary p-5 text-base font-semibold text-foreground">
            По нашей статистике, 8 из 10 региональных предпринимателей прямо сейчас находятся
    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-[#04140f] via-[#0a2e22] to-[#0d4d3a] py-12 md:py-24 text-white">
      <Container>
        ...
        <h2 className="mt-6 max-w-4xl text-4xl font-black tracking-tight md:text-6xl">
          С чем собственник выходит из цеха пересборки Елены Кремневой
        </h2>
...
const faqCategories: FaqCategory[] = [
  {
    ...
    qa: [
      ...
      { q: "Подойдёт ли моему бизнесу?", a: "Да, если у вас уже есть действующий бизнес и вы готовы внедрять, а не просто слушать. На диагностике честно скажем, если нет." },
      { q: "Сколько длится обучение?", a: "Зависит от выбранной программы — от двухдневных интенсивов до полноценного курса. Подберём формат под ваши задачи на диагностике." },
      { q: "Какой формат занятий?", a: "Очные встречи в Липецке + экскурсии к предпринимателям регионов + онлайн-разборы. Раздаточные материалы и записи остаются у вас навсегда." },
    ],
  },
  {
    ...
    tone: "gray",
    qa: [
      { q: "За сколько окупится участие?", a: "Если будете внедрять и применять полученные навыки — окупится быстро, за счёт первой же системной перестройки найма, продаж или ценообразования. Если придёте просто «потусоваться» — не окупится никогда. Решает не курс, а ваши действия после него." },
      ...
      { q: "Чем отличаетесь от других программ?", a: "Мы работаем точечно с собственниками из регионов и внедряем в ваш бизнес, а не учим «вообще»." },
  ...
  },
  {
    ...
    preview: "Кто ведёт · Как устроен переход между этапами",
    tone: "green",
    qa: [
      { q: "Кто ведёт программу?", a: "Елена Кремнева и команда — представители практики бизнес-историй: действующие предприниматели, методологи и эксперты с опытом в производстве, рознице и услугах. Каждый — признанный практик в своей области, не просто теоретик." },
      { q: "Как устроен переход между этапами?", a: "Не внедрили — не идём дальше. Пока вы не закрыли этап реальным артефактом внедрения в своём бизнесе, к следующему уровню не переходите. Это жёсткое правило: программа двигается со скоростью внедрения, а не со скоростью лекций." },
    ],
  },
  {
    ...
    tone: "gray",
    qa: [
      { q: "Берёте ли вы всех?", a: "Нет. Сначала бесплатные диагностика и собеседование. Если увидим, что обучение вам не подходит — честно сообщим." },
      { q: "Кому не подойдёт обучение?", a: "Тем, кто ищет волшебную таблетку и не готов внедрять и работать." },
    ],
  },
];
